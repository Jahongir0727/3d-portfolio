...
const Navbar = () => {
  ...
  return (
    <nav className={`${styles.paddingX} w-full flex items-center py-5 fixed top-0 z-20 bg-primary`}>
      <div className="w-full flex justify-between items0center max-w-7xl mx-auto">
        <Link to = "/" className="flex items-center" onClick={() => {setActive(""); window.scrollTo(0,0);}}>
          <img src={logo} alt="logo" className="w-11 h-10 object-contain" />
          <p className="text-white text-[18px] font-bold cursor-pointer flex">
            Jay &nbsp;
            <span className='sm:block hidden'>| JS</span>
            </p>
        </Link>
